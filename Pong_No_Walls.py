...
while p1_wins < 2 or p2_wins < 2:
    ...
    for b in balls:
        ...
        if player1_top.colliderect(ball):
            ...
            if (ball.top < 0 or ball.bottom > window_height) and (ball.left > (window_width / 2) or ball.right < window_width):
                out_of_bounds.play()
                if player2_score < 10:
                    player2_score += 1
        ...
        if player2_top.colliderect(ball):
            bounce_sound.play()
            if ball.top < player2_top.bottom:
                ...
                if direction == UPRIGHT:
                    direction = DOWNRIGHT
        if player2_bottom.colliderect(ball):
            bounce_sound.play()
            if ball.bottom > player2_bottom.top:
                ...
                if direction == DOWNRIGHT:
                    direction = UPRIGHT
        if player2_side.colliderect(ball):
            bounce_sound.play()
